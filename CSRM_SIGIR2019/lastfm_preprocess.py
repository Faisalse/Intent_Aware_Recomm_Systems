...
with open("data/lastfm_info.csv") as f:
...
# collapse repeating items [1,2,2,2,4,5,5,6,7,7,4] after [1,2,4,5,6,7,4]
new_sess_clicks = {}
for s in sess_clicks.keys():
    # take first item of a session
...
for s in sess_clicks1.keys():
    if len(sess_clicks1[s]) == 1 or len(sess_clicks1[s]) > 50:
        del sess_clicks[s]
        del sess_date[s]
...
# prepare data deep learning model
# Convert training sessions to sequences and renumber items to start from 1
for s, date in train_sess:
    seq = sess_clicks[s]
    outseq = []
    for i in seq:
        if i in item_dict_new:
...
pickle.dump((tr_seqs, tr_labs), open('data/lastfm_train.pkl', 'wb'))
pickle.dump((valid_seqs, valid_labs), open('data/lastfm_valid.pkl', 'wb'))
pickle.dump((te_seqs, te_labs), open('data/lastfm_test.pkl', 'wb'))
print('Done!')

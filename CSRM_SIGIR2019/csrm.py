...
from CSRM_SIGIR2019.ome import OME
from CSRM_SIGIR2019.data_process import *
import time
import pickle
from CSRM_SIGIR2019.accuracy_measures import *
...
class CSRM:
    ...
    def get_minibatches_idx(self, n, minibatch_size, shuffle=False):
        ...
        return len(minibatches), zip(range(len(minibatches)), minibatches)

    def pred_evaluation(self, data, iterator, ntm_init_state, topKList = [10, 20]):
        """
        ...
        for _, valid_index in iterator:
            ...
            for i in range(len(pred)):
                partitioned_indices = np.argpartition(-pred[i], 30)[:30]
                top_scores = pred[i][partitioned_indices]
                recommendation_list = pd.Series(top_scores, index = [index_ + 1  for index_ in partitioned_indices])
                recommendation_list.sort_values(ascending = False, inplace = True)
                for key in performance_measures:
                    performance_measures[key].add(recommendation_list, batch_label[i])
        return performance_measures, ntm_init_state


    def construct_feeddict(self, batch_data, batch_label, keepprob, state, starting=False):
        x, mask, y, lengths = prepare_data(batch_data, batch_label)
    ...
    def train(self, Train_data, Validation_data, number_epoch = 1, validation = False, stopping_patience = 5):
        
        self.sess.run(tf.global_variables_initializer())
        length_of_kf_zip_object ,kf = self.get_minibatches_idx(len(Train_data[0]), self.batch_size)
        _, kf_valid = self.get_minibatches_idx(len(Validation_data[0]), self.batch_size)
        uidx = 0
        bad_count = 0
        estop = False
        ...
        for epoch in range(number_epoch):
            ...
            # 训练
            print('*****************************************************************')
            for _, train_index in tqdm(kf, total=length_of_kf_zip_object):
                uidx += 1
            ...
            if validation:
                performance_measures, session_memory_state = self.pred_evaluation(Validation_data, kf_valid, session_memory_state)
